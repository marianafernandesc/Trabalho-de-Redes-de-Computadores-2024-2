...
def fetch_webpage(net, times=3):
    ...
    for i in range(times):
        cmd = "curl -o /dev/null -s -w '%%{time_total}' http://%s/index.html" % h1.IP()
        result = h2.cmd(cmd).strip()
        final_result+=result
        print("Webpage fetch time %d: %s seconds" % (i + 1, final_result))
...
def bufferbloat():
    ...
    start_time = time()
    while True:
        fetch_times.append(fetch_time(net, 3))
        sleep(5)
        now = time()
    ...
    # CLI(net)
    average_time = statistics.mean(fetch_times)
    std_deviation = statistics.stdev(fetch_times)

    print("Average fetch time: %.3f seconds" % average_time)
    print("Standard deviation of fetch times: %.3f seconds" % std_deviation)
