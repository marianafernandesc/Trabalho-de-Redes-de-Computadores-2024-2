...
images = ["q_20.png", "q_100.png", "rtt_q20.png", "rtt_q100.png"]
...
def fetch_image(net):
    ...
    result = 0
    for i in range(3):
        img = images[random.randint(0, len_images-1)]
        # The following is an attempt to curl and pring out the url to verify it is working properly
        #proc = h2.popen("curl " + str(ip) + "/http/index.html", shell=True, text=True, stdout=PIPE)
        proc = h2.popen("curl -o /dev/null -s -w %{time_total} http://" + str(ip) + ":8080/" + img, shell=True, text=True, stdout=PIPE)
